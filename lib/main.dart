  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: <Widget>[
          Expanded(
            //can expand to fillout entire parent
            child: Container(
...
class MainState extends InheritedWidget {
  ...
  final double prevValue; //save first value that users type  into calculator
  final String value; //next go between, value fr the second
  final String
      op; //operator characters for holding operator tokens from buttons
  final Function
      onPressed; //use to reserve our button pressess inside our layout

  static MainState of(BuildContext context) {
    return context.inheritFromWidgetOfExactType(
        MainState); //rerenders old instance with new
  }
  ...
  void onPressed(keyvalue) {
    switch (keyvalue) {
      case "C":
        ...
        prevValue = 0.0;
        value = "";
        setState(() => inputString = ""); //to update our ui
        break; //so it doesnt runthrough other cases
      case ".":
      case "%":
      case "<=":
      case "+/-":
